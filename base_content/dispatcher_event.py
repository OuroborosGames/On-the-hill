...
def dispatch(stories):
    def action(state):
        for story, predicate in stories:
            if predicate(state):
                text_events.spawn_after_n_turns(state, story, randint(12, 24))
                return
    return action
...
class DispatcherEvent(text_events.TextEventPrototype):
    ...
    @staticmethod
    def should_be_activated(state):
        return True if state.turn == 600 else False
